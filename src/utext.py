...
'''
    ...
    def init_menu(self):
        ...
        self.fileformat.append(Gtk.SeparatorMenuItem())
        #
        self.formats['bloque_quote'] = Gtk.ImageMenuItem.new_with_label(
            _('Bloque Quote'))
        self.formats['bloque_quote'].connect(
            'activate', self.on_toolbar_clicked, 'bloque_quote')
    ...
    def on_buffer_changed(self, widget):
        ...
        if (self.number_of_lines != self.writer.get_buffer().get_line_count())\
            ...
            if not self.launched or self.launched is False:
                ...
                print('launched')
                GObject.timeout_add(
                    300*1000, self.save_current_file_deferreaded)

    def save_current_file_deferreaded(self):
        if self.is_saving:
            return False
        if self.preferences['autosave'] and self.file_saved is False:
            self.number_of_lines = self.writer.get_buffer().get_line_count()
            self.save_current_file()
            print('saved')
        self.launched = False
